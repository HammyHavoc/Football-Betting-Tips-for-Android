...
define([
	...
	if ( view_template === 'home') { //Don't need .html here
		...
		//Get tips posts from our "tips" component and keep only 3 of them to display in template:
		template_args.tips_posts = _.first( TemplateTags.getComponent('tips').view_data.posts.toJSON(), 3 );

		//Same with offers posts, from our "offers" component:
		template_args.offers_posts = _.first( TemplateTags.getComponent('offers').view_data.posts.toJSON(), 3 );

		//Now "tips_posts" and "offers_posts" variables are available in our home.html template :)
	}
